...
Для проверки работы сайта на продакшн-домене (https://kittypoopvpn.ru) используются e2e тесты на Playwright.
...
2. Запустите тесты:
   ```
   npx playwright test test/e2e-pricing.spec.js
   ```

Тест проверяет:
- Открытие страницы тарифов
- Видимость всех тарифов
- Переход к оплате по кнопке "ВЫБРАТЬ"

Конфиг: `playwright.config.js` (базовый домен уже прописан) 